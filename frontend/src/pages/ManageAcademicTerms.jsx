...
function ManageAcademicTerms({ refresh, refreshTrigger }) {
  ...
  const [formData, setFormData] = useState({ term_name: 'Fall', school_year: '', start_date: '', end_date: '' });
  ...
  async function handleSubmit(e) {
    ...
    try { await axios.post('/api/academic-term', formData); setShowModal(false); refresh(); }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <button onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', backgroundColor: '#111827', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}><Plus size={16} /> Add Term</button>
      </div>
      ...
      {showModal && (
        <Modal title="Add Academic Term" onClose={() => setShowModal(false)}>
